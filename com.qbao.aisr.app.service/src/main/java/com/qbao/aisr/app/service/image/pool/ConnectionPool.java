...
public class ConnectionPool {
	...
	public TrackerServer checkout(String logId) throws FastDFSException {
		...
		if (trackerServer == null) {
			...
			if (nowPoolSize < maxPoolSize) {
				...
				LOGGER.error("当前创建连接数 nowPoolSize＝"+nowPoolSize+"大于maxPoolSize:"+maxPoolSize);
			}
			if (trackerServer == null) {
				LOGGER.error("[获取空闲连接(checkout)-error][" + logId
	...
	private void initClientGlobal() throws Exception {
		String configFilePath =ConnectionPool.class.getResource("/fdfs_client.conf").getPath();
		LOGGER.info("fastDFS config path="+configFilePath);
